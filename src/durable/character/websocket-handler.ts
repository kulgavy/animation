...
export class WebSocketHandler {
    ...
    async handleWebSocket(req: Request, ctx: DurableObjectState) {
        try {
            ...
            await this.logger.log('WebSocketHandler', `WebSocket connection established: ${connectionData.clientId}`);
            ...
            await this.logger.error('WebSocketHandler', `Failed to establish WebSocket connection: ${error}`);
    ...
    async handleMessage(ws: WebSocket, message: string) {
        ...
        try {
            ...
            await this.broadcastUpdate(command, { characters: this.characterManager.getAllCharacters(), result });
        } catch (error) {
    ...
    private async broadcastUpdate(command: Command, data: unknown) {
        ...
        for (let i = 0; i < activeConnections.length; i += CHUNK_SIZE) {
            ...
            await Promise.allSettled(chunk.map(async (socket) => {
                try {
                    socket.send(response);
                } catch (error) {
                    await this.logger.error(
                        'WebSocketHandler',
                        `Failed to broadcast to socket: ${error}`
                    );
                }
            }));
        }
